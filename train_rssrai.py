...
import utils_rssrai
from models_rssrai import HyperParams
import models_rssrai
...
class Image:
    ...
    def dist_mask(self):
        if self._dist_mask is None:
            ...
            self._dist_mask = (
                np.stack([utils_rssrai.dist_mask(m, max_dist=5) for m in self.mask])
                .astype(np.float16))
...
class Model:
    def __init__(self, hps: HyperParams):
        self.hps = hps
        self.net = getattr(models_rssrai, hps.net)(hps)
    ...
    def _init_optimizer(self, lr):
        # optim.Adam(self.net.parameters(),lr=lr, weight_decay=self.hps.weight_decay)
        return optim.SGD(self.net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0001)
    ...
    def train(self, logdir: Path, train_ids: List[str], valid_ids: List[str],
        ...
        for n_epoch in range(start_epoch, self.hps.n_epochs):
            lr_pow = 0.9
            lr = self.hps.lr*((1. - float(n_epoch) / self.hps.n_epochs) ** lr_pow)
            self.optimizer = self._init_optimizer(lr)
    ...
    def preprocess_image(self, im_data: np.ndarray) -> np.ndarray:
        ...
        mean = np.array([134.93844588,  97.15064623, 102.4617644,   94.20321606],dtype=np.float32)
        std = np.array([58.23315644, 57.70064763, 54.32877434, 53.13143188], dtype=np.float32)
    ...
    def load_image(self, im_id: str) -> Image:
        ...
        if im_data_path.exists():
            ...
            im_data = self.preprocess_image(utils_rssrai.load_image(im_id))
        ...
        if mask_path.exists() and not pre_buffer:
            ...
            mask = utils_rssrai.load_mask(im_id)
            # mask = np.array(
            #     [utils_rssrai.mask_for_polygons(im_size, poly_by_type[cls])
        ...
        def gen_batch(_):
            ...
            for _ in range(self.hps.batch_size):
                ...
                if self.hps.augment_rotations:
                    ...
                    patch = utils_rssrai.rotated(patch, angle)
                    mask = utils_rssrai.rotated(mask, angle)
                    if self.hps.needs_dist:
                        dist_mask = utils_rssrai.rotated(dist_mask, angle)
    ...
    def _train_on_feeds(self, gen_batch, n_batches: int, no_mp: bool):
        ...
        map_ = (map if no_mp else
                partial(utils_rssrai.imap_fixed_output_buffer, threads=4))
    ...
    def _jaccard_stats(self):
        return {cls: {threshold: [[] for _ in range(4)]
    ...
    def _update_jaccard(self, stats, mask, pred):
        ...
        for cls, tp_fp_fn_tn in stats.items():
            ...
            if len(mask.shape) == 3:
                ...
                p, y = pred[:, cls_idx], mask[:, cls_idx]
            for threshold, (tp, fp, fn, tn) in tp_fp_fn_tn.items():
                _tp, _fp, _fn, _tn = utils_rssrai.mask_tp_fp_fn_tn(p, y, threshold)
                tp.append(_tp)
                fp.append(_fp)
                fn.append(_fn)
                tn.append(_tn)
    ...
    def _log_jaccard(self, stats, prefix=''):
        ...
        for cls, tp_fp_fn_tn in stats.items():
            for threshold, (tp, fp, fn, tn) in tp_fp_fn_tn.items():
                jaccard = self._jaccard(tp, fp, fn, tn)
                self._log_value(
    ...
    def _log_im(self, xs: np.ndarray,
        ...
        for i, (x, y, p) in enumerate(zip(xs, ys, pred_ys)):
            ...
            x = utils_rssrai.scale_percentile(x.transpose(1, 2, 0))
            channels = [x[:, :, :3]]  # RGB
            if x.shape[-1] == 12:
                channels.extend([
                    x[:, :, 4:7],    # M
    ...
    def validate_on_images(self, valid_images: List[Image],
        ...
        for im in valid_images:
            ...
            for xy_batch in utils_rssrai.chunks(all_xy, self.hps.batch_size // 2):
                inputs = np.array(
        ...
        def gen_batch(xy_batch_):
            ...
            for x, y in xy_batch_:
                ...
                for i in range(1, n_rot):
                    inputs_.append(utils_rssrai.rotated(patch, i * 90))
            return xy_batch_, np.array(inputs_, dtype=np.float32)

        for xy_batch, inputs in utils_rssrai.imap_fixed_output_buffer(
                gen_batch, tqdm.tqdm(list(
                    utils_rssrai.chunks(all_xy, self.hps.batch_size // (4 * n_rot)))),
                threads=2):
            y_pred = self.net(self._var(torch.from_numpy(inputs)))
            for idx, mask in enumerate(y_pred.data.cpu().numpy()):
                ...
                i = idx % n_rot
                if i:
                    mask = utils_rssrai.rotated(mask, -i * 90)
                # mask = (mask >= 0.5) + 0.001
