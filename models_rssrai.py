...
@attr.s(slots=True)
class HyperParams:
    classes = attr.ib(default=list(range(16)))
    net = attr.ib(default='UNet')
    n_channels = attr.ib(default=4)  # max 20
    total_classes = 16
    thresholds = attr.ib(default=[0.5])
    pre_buffer = attr.ib(default=0.0)

    patch_inner = attr.ib(default=128)
...
class UNet(BaseNet):
    ...
    def forward(self, x):
        ...
        b = self.hps.patch_border
        return torch.sigmoid(x_out[:, :, b:-b, b:-b])
...
class DeepLabV3Plus(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(h[:, :, b:-b, b:-b])
