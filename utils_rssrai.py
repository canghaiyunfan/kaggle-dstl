...
def load_image(im_id: str, rgb_only=False, align=True) -> np.ndarray:
    image_path = '/data/data_gWkkHSkq/kaggle-dstl/rssrai_dataset/train/{}.tif'.format(im_id)
...
def load_mask(im_id:str):
    image_path = '/data/data_gWkkHSkq/kaggle-dstl/rssrai_dataset/train/{}_label.tif'.format(im_id)
    tif = TIFF.open(image_path, mode="r")
    im = tif.read_image()

    onehot_mask = rgb_to_onehot(im,COLOR_DICT)

    return onehot_mask.transpose([2,0,1])
...
def mask_tp_fp_fn_tn(pred_mask: np.ndarray, true_mask: np.ndarray,
